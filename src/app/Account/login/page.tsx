      ...
      const login= async function(data:any){
       try {
          const response=  await axios.post("http://localhost:3000/api/user/login",data);
            ...
            console.log("error while log in",error.response?error.response.data: error.message)
        }

       }
    
      // const post= async function(data:any){
      //   try {
      //      const currentUSer:any= await axios.get("http://localhost:3000/api/user/currentuser")
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="divide-y divide-gray-200">
              <div className="py-8 text-lg leading-6 space-y-4 text-black sm:text-lg sm:leading-7">
                ...
                <div className="relative">
                  <button className="bg-[#00BA7C] text-white rounded-md px-2 py-1">Submit</button>
                </div>
              </div>
            </div>
          </div>
